...
import com.alipay.sofa.registry.server.session.providedata.FetchMultiPushService;
...
public class PushSwitchService {
  ...
  @Resource private FetchMultiPushService fetchMultiPushService;
  ...
  public boolean canPushMulti(String dataInfoId, Set<String> dataCenters) {
    ...
    for (String dataCenter : dataCenters) {
      if (!dataInfoIdCanPush(dataInfoId, dataCenter) || !dataCenterCanPush(dataCenter)) {
        return false;
      }
    }
    return true;
  ...
  public boolean canIpPushMulti(String ip, String dataInfoId, Set<String> dataCenters) {
    ...
    for (String dataCenter : dataCenters) {
      if (!dataInfoIdCanPush(dataInfoId, dataCenter) || !dataCenterAndIpCanPush(dataCenter, ip)) {
        return false;
      }
    }
    return true;
  ...
  private boolean dataInfoIdCanPush(String dataInfoId, String dataCenter) {
    return sessionServerConfig.isLocalDataCenter(dataCenter)
        || fetchMultiPushService.multiPush(dataInfoId);
  }
