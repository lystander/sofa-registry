...
public class PushDataGenerator {
  ...
  public PushData createPushData(MultiSubDatum unzipDatum, Map<String, Subscriber> subscriberMap) {
    ...
    if (multi) {
      return createMultiReceivePushData(unzipDatum, clientCell, Lists.newArrayList(subscriberMap.keySet()), subscriber, compressorGetter);
    }
    return createReceivePushData(unzipDatum, clientCell, Lists.newArrayList(subscriberMap.keySet()), subscriber, compressorGetter);
  }

  private PushData createMultiReceivePushData(MultiSubDatum unzipDatum, String clientCell,
                                              List<String> subscriberRegisterIdList,
                                              Subscriber subscriber, CompressorGetter compressorGetter) {
    ...
    Map<String, Set<String>> segmentZones = dataCenterMetadataCache.dataCenterZonesOf(pushDataCenters);
    ...
    if (unzipDatum.getDatumMap().size() != segmentZones.size()
      ...
      throw new SofaRegistryRuntimeException(StringFormatter.format(
              "createMultiReceivePushData error, datum.dataCenters: {}, metadata.dataCenters: {}",
              pushDataCenters, metadataDataCenters));
    }
  ...
  private void fillSegmentPushInfo(PushData<MultiReceivedData> pushData,
  ...
  private PushData createReceivePushData(MultiSubDatum unzipDatum, String clientCell,
                                         List<String> subscriberRegisterIdList, Subscriber subscriber, CompressorGetter compressorGetter) {
    Predicate<String> pushDataPredicate =
            ZonePredicate.pushDataPredicate(
                    unzipDatum.getDataId(), clientCell, subscriber.getScope(), sessionServerConfig);
